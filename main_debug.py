"""Debug version to get actual LLM responses."""
...
for module in ["google", "httpx", "vertexai", "matplotlib", "pandas", "numpy"]:
...
class DebugLLMApp:
    """Debug LLM application to see actual responses."""
    ...
    async def generate_content(self, prompt: str) -> str:
        ...
        try:
            print(f"🔍 Debug: Creating request for prompt: {prompt}")
            ...
            print(f"🔍 Debug: Request created: {request}")
            
            use_case = self.container.get_generate_content_use_case()
            print(f"🔍 Debug: Use case created: {use_case}")
            ...
            if response.content and response.content.strip():
                ...
                return f"LLM応答が空でした。フォールバック応答: {prompt}についてお答えします。"
    ...
    async def generate_with_functions(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            print(f"🔍 Debug: Function calling for prompt: {prompt}")
            ...
            response = await use_case.execute(request)
            print(f"🔍 Debug: Function calling response: {response}")
            ...
            print(f"🔍 Debug: Function calling exception: {e}")
            return {
                "content": f"「{prompt}」について、利用可能な機能を使って回答いたします。",
                "function_calls": [],
                "function_results": [],
...
async def main():
    """Main function for debugging LLM responses."""
    try:
        ...
        print(f"\n{'='*50}")
        print(f"Test 1: Basic Content Generation")
        print(f"{'='*50}")
        ...
        print(f"\n{'='*50}")
        print(f"Test 2: Function Calling")
        print(f"{'='*50}")
        ...
        print(f"\n{'='*50}")
        print(f"Test 3: Data Analysis Workflow")
        print(f"{'='*50}")
